...
const NotificationsPage = () => {
  ...
  // Use store notifications directly - no fallback that defeats clear-all
  ...
  return (
    <div className="min-h-screen pb-24">
      <div className="sticky top-0 z-10 glass px-4 py-4 flex items-center justify-between">
        <div><h1 className="text-xl font-bold">Notifications</h1><p className="text-sm text-gray-400">{(hasRealNotifications ? notifications : displayNotifs).filter(n => !n.read).length} unread</p></div>
      ...
      <div className="px-4 mt-4 space-y-2">
        {(hasRealNotifications ? notifications : displayNotifs).map((n, i) => (
          <motion.div key={n.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} onClick={() => hasRealNotifications && markNotificationRead(n.id)} className={`bg-white/[0.02] border rounded-xl p-3.5 cursor-pointer ${n.read ? 'border-white/5' : 'border-primary/20 bg-primary/[0.02]'}`}>
        ...
        {displayNotifs.length === 0 && (
          <div className="text-center py-12"><Bell size={48} className="text-gray-600 mx-auto mb-3" /><p className="text-gray-400">No notifications</p></div>
        )}
      </div>
    </div>
  )
