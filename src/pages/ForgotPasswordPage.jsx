  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm space-y-6">
        ...
        <div className="relative"><Mail size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" /><input type="email" value={email} onChange={e => { setEmail(e.target.value); setError('') }} placeholder="Your email" className={`w-full bg-white/5 border ${error ? 'border-red-500' : 'border-white/10'} rounded-xl pl-10 pr-4 py-3 text-sm focus:outline-none focus:border-primary`} /></div>
        ...
        <button onClick={() => {
          ...
          setSent(true)
        }} className="w-full py-3 bg-gradient-to-r from-primary to-primary-dark rounded-xl font-bold">Send Reset Link</button>
      </motion.div>
    </div>
  )
}
